import { isString, isDate, isObject, has, isNumber, isObjectLike, isBoolean, isArray } from 'lodash';
import { isJobEvent } from '../events/validators';
...
export function isJobDefinition(input: any): input is JobDefinition {
  return isObjectLike(input) && isString(input.service) && isString(input.name) && isString(input.id);
}
...
export function isJob(input: any): input is Job {
  ...
  const hasUpdates =
    isArray(input.updates) && (input.updates as any[]).reduce((acc, val) => acc && isJobEvent(val), true);

  const hasErrors = isArray(input.errors) && (input.errors as any[]).reduce((acc, val) => acc && isString(val), true);

  logger.debug({ hasDefinition, hasJobProps, hasUpdates });
