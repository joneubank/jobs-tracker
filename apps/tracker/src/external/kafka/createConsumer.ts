import config, { KafkaConsumerConfiguration } from '../../config';
import { Consumer, Kafka, KafkaMessage, Producer } from 'kafkajs';
import Logger from 'logger';
const logger = Logger('Kafka.createConusmer');
...
function createConsumer(
  config: KafkaConsumerConfiguration,
  messageHandler: (message: KafkaMessage, sendDlqMessage: (messageJSON: string) => Promise<void>) => Promise<void>,
   ...
   * @param message
   */
  async function handleMessage(message: KafkaMessage, sendDlqMessage: (messageJSON: string) => Promise<void>) {
    try {
